...
class MemoryAllocator:
    ...
    def alloc(self, need_size):
        ...
        # print(f"mem_state: {self.mem_state}")
        # print(f"mem_cum_sum: {self._mem_cum_sum}")
        select_index = torch.logical_and(self._mem_cum_sum <= need_size, self.mem_state == 1)
        # print(select_index) # torch.Size([10000]) False/True
        
        # print(self.indexes) # torch.Size([10000])
        
        # # Save select_index and indexes to txt files
        # with open('/data01/tuwenming/S-LoRA/log/select_index.txt', 'w') as f:
        #     for item in select_index.tolist():
        #         f.write(f"{item}\n")
        
        # with open('/data01/tuwenming/S-LoRA/log/indexes.txt', 'w') as f:
        #     for item in self.indexes.tolist():
        #         f.write(f"{item}\n")
        
        index = self.indexes[select_index]
        self.mem_state[index] = 0
        self.can_use_mem_size -= len(index)
        return index
    ...
    def reset_all_pool(self):
        ...
        print(f"len(self.indexes){len(self.indexes)}")
        self.can_use_mem_size = self.tot_size
        # self.can_use_mem_size_prefix = self.cache_size
        # self.can_use_mem_size_suffix = self.tot_size - self.cache_size
        self.key_buffer = torch.empty((self.layer_num, self.tot_size, self.head_num, self.head_dim),
                                       dtype=self.dtype, device="cuda")
        self.value_buffer = torch.empty((self.layer_num, self.tot_size, self.head_num, self.head_dim),
                                       dtype=self.dtype, device="cuda")
