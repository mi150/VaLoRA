...
from valora.common.basemodel.layer_weights.hf_load_utils import load_hf_weights
from valora.common.basemodel.infer_struct import InferStateInfo
...
from valora.mprophet.model_config import get_config_json
...
class TpPartBaseModel:
    ...
    def _init_weights(self):
        ...
        # # 对每层的 q, k, v, o 权重进行连接
        layer_weights = [
            torch.cat([layer.q_weight_, layer.k_weight_, layer.v_weight_, layer.o_weight_], dim=0)
            for layer in self.trans_layers_weight
        ]

        # # 将所有层的权重连接在一起
        self.all_weights = torch.cat(layer_weights, dim=0)
        return 
