...
from valora.common.mem_manager import MemoryManager
from valora.utils.infer_utils import mark_start, mark_end
...
class InferBatch:
    ...
    @classmethod
    @torch.no_grad()
    def init_batch(cls, batch_id, requests, dtype: torch.dtype, device: torch.device, mem_manager:MemoryManager, vocab_size: int, num_problems: int):
    ...
    def merge(cls, batch1, batch2):
        ...
        num_problems = [0, [0, 0]]
        num_problems[0] = batch1.num_problems[0] + batch2.num_problems[0]
        num_problems[1][0] = batch2.num_problems[1][0]
        num_problems[1][1] = batch2.num_problems[1][1]
        requests_idx_mapping = {}
