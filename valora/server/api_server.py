...
from .visualserver.manager import start_visual_process # process visual message 
from .multimodal_params import MultimodalParams
from valora.utils.net_utils import alloc_can_use_network_port
...
from valora.utils.start_utils import start_submodule_processes
...
from valora.mprophet.lora_stats import LoRAProphet

from valora.server import multimodal_params
...
async def generate_stream(request: Request) -> Response:
    ...
    # adapter_dir = request_dict["lora_dir"] if "lora_dir" in request_dict else None
    adapter_dir = request_dict.get("lora_dir") or request_dict.get("adapter_dir") 
    ...
    # print("-------------------", sample_params_dict)
    sampling_params.verify()
    multimodal_params = MultimodalParams(request_dict['multimodal_params']['images'])
    # multimodal_params = []
    if "req_id" in request_dict:
        request_id = request_dict["req_id"]
    else:
        request_id = uuid.uuid4().hex
    results_generator = httpserver_manager.generate(adapter_dir, prompt, sampling_params, request_id, multimodal_params=multimodal_params)
    # print("generator:", adapter_dir, request_id)
    # Streaming case
...
def main():
    ...
    parser.add_argument("--max_total_token_num", type=int, default=50000,
                        help="the total token nums the gpu and model can support, equals = max_batch * (input_len + output_len)")
    parser.add_argument("--batch_max_tokens", type=int, default=50000,
                        help="max tokens num for new cat batch, it control prefill batch size to Preventing OOM")
    parser.add_argument("--eos_id", type=int, default=2,
                        help="eos stop token id")
    parser.add_argument("--running_max_req_size", type=int, default=64,
    ...
    parser.add_argument("--nccl_port", type=int, default=28767,
    ...
    parser.add_argument("--data_type",type=str,choices=["fp16", "float16", "bf16", "bfloat16", "fp32", "float32"],
    ...
    parser.add_argument("--scheduler", type=str, default="ours")
    ...
    # modify nccl_port
    config_file_path = '../valora/common/configs/config.py'
    with open(config_file_path, 'r') as file:
        lines = file.readlines()
    with open(config_file_path, 'w') as file:
        for line in lines:
            if "nccl_port" in line:
                file.write(f'    "nccl_port": {args.nccl_port}\n')
            else:
                file.write(line)
    ...
    router_port, detokenization_port, httpserver_port, cache_port, visual_port = can_use_ports[0:5]
