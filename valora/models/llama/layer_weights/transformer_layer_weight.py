...
class LlamaTransformerLayerWeight(TransformerLayerWeight):
    ...
    def _load_qkvo_weights(self, weights):
        ...
        if f"model.layers.{self.layer_num_}.self_attn.q_proj.weight" in weights:
            ...
            print(f"Loading q_weight_ for layer {self.layer_num_}")
        else:
            print(self.layer_num_)
        if f"model.layers.{self.layer_num_}.self_attn.k_proj.weight" in weights:
            ...
            print(f"Loading k_weight_ for layer {self.layer_num_}")
        else:
            print(self.layer_num_)
        ...
        if f"model.layers.{self.layer_num_}.self_attn.v_proj.weight" in weights:
            ...
            print(f"Loading v_weight_ for layer {self.layer_num_}")
        else:
            print(self.layer_num_)   
        ...
        if self.q_weight_ is not None and self.k_weight_ is not None and self.v_weight_ is not None:
            self.qkv_weight_ = torch.cat([self.q_weight_, self.k_weight_, self.v_weight_], dim=1)
        # else:
        #     raise ValueError("One or more of q_weight_, k_weight_, or v_weight_ is None.")
