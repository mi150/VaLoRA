...
from valora.models.qwen.triton_kernel.context_flashattention_nopad import context_attention_fwd
from valora.models.qwen.triton_kernel.token_attention_nopad_att1 import token_att_fwd, token_att_fwd_int8k
from valora.models.qwen.triton_kernel.token_attention_nopad_softmax import token_softmax_fwd
from valora.models.qwen.triton_kernel.token_attention_nopad_reduceV import token_att_fwd2, token_att_fwd2_int8v
from valora.models.qwen.triton_kernel.rmsnorm import rmsnorm_forward
from valora.models.qwen.triton_kernel.rotary_emb import rotary_emb_fwd

from valora.models.qwen.infer_struct import QwenInferStateInfo
from valora.common.basemodel.triton_kernel.destindex_copy_kv import destindex_copy_kv, destindex_copy_quantize_kv
from valora.common.basemodel import TransformerLayerInferTpl

class QwenTransformerLayerInfer(TransformerLayerInferTpl):
    """
    """
    ...
    def _att_norm(self, input, infer_state: QwenInferStateInfo, layer_weight: QwenTransformerLayerWeight) -> torch.Tensor:
        return rmsnorm_forward(input, weight=layer_weight.att_norm_weight_, eps=self.eps_)
    
    def _ffn_norm(self, input, infer_state: QwenInferStateInfo, layer_weight: QwenTransformerLayerWeight) -> torch.Tensor:
        return rmsnorm_forward(input, weight=layer_weight.ffn_norm_weight_, eps=self.eps_)

    def _get_qkv(self, input, cache_k, cache_v, infer_state: QwenInferStateInfo, layer_weight: QwenTransformerLayerWeight) -> torch.Tensor:
    ...
    def _post_cache_kv(self, cache_k, cache_v, infer_state: QwenInferStateInfo, layer_weight: QwenTransformerLayerWeight):
    ...
    def _context_attention_kernel(self, q, k, v, infer_state: QwenInferStateInfo, layer_weight) -> torch.Tensor:
    ...
    def _token_attention_kernel(self, q, infer_state: QwenInferStateInfo, layer_weight) -> torch.Tensor:
        return self._token_decode_attention_mode(q, infer_state)

    def _get_o(self, input, infer_state: QwenInferStateInfo, layer_weight: QwenTransformerLayerWeight) -> torch.Tensor:
        o_tensor = torch.mm(input.view(-1, self.tp_o_head_num_ * self.head_dim_), layer_weight.o_weight_)
        return o_tensor

    def _ffn(self, input, infer_state: QwenInferStateInfo, layer_weight: QwenTransformerLayerWeight) -> torch.Tensor:
    ...
    def _token_decode_attention_normal(self, q, infer_state: QwenInferStateInfo):
        ...
        if triton.__version__ == "2.0.0":
            ...
            o_tensor = torch.empty_like(q)
            from valora.models.qwen.triton_kernel.token_attention_softmax_and_reducev import token_softmax_reducev_fwd
    ...
    def _token_decode_attention_int8kv(self, q, infer_state: QwenInferStateInfo):
        total_token_num = infer_state.total_token_num
        batch_size = infer_state.batch_size
        calcu_shape1 = (batch_size, self.tp_q_head_num_, self.head_dim_)
        att_m_tensor = torch.empty((self.tp_q_head_num_, total_token_num), dtype=q.dtype, device="cuda")
    ...
    def _token_decode_attention_mode(self, q, infer_state: QwenInferStateInfo):
        if "int8kv" in self.mode:
