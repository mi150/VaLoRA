...
from valora.common.basemodel import PostLayerInferTpl

class QwenPostLayerInfer(PostLayerInferTpl):
    """
    """
    ...
    def _norm(self, input, infer_state, layer_weight: QwenPreAndPostLayerWeight) -> torch.Tensor:
    ...
    def token_forward(self, input_embeddings, infer_state: QwenInferStateInfo, layer_weight: QwenPreAndPostLayerWeight, return_logics=False):
        batch_size = infer_state.batch_size
        last_input = torch.empty((batch_size, self.embed_dim_), device=input_embeddings.device, dtype=torch.float16)
        if infer_state.is_prefill:
            last_index = torch.cumsum(infer_state.b_seq_len, dim=0, dtype=torch.long) - 1
            last_input[:, :] = input_embeddings[last_index, :]
        else:
            last_input[:, :] = input_embeddings[-batch_size:, :]
