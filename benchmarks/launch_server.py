...
if __name__ == "__main__":
    ...
    parser.add_argument("--backend", type=str, default="valora",
    ...
    parser.add_argument("--nccl_port", type=int, default=28769)
    ...
    if args.device == "a10g":
        if args.num_adapter is None:
            args.num_adapter = 200
        if args.num_token is None:
            args.num_token = 14000
    elif args.device == "h100":
        if args.num_adapter is None:
            args.num_adapter = 1000
        if args.num_token is None:
            args.num_token = 120000
    ...
    if args.backend == "ours" or "valora":
        cmd = f"python -m valora.server.api_server --max_total_token_num {args.num_token}"
    ...
    cmd += " --eos_id 151643" # especial to qwen model
    # print(cmd)
    cmd += " --enable_multimodal" # support multimodal input
    # cmd += " --cache_capacity"
    cmd += f" --port {args.port}"
    cmd += f" --nccl_port {args.nccl_port}"
    os.system(cmd)
