...
"""
input request rate， csv「req/s」

output req arrival time

"""


def get_arrival_times(req_rate,
                      file_path='real_trace/AzureLLMInferenceTrace_code_1week.csv'):
    ...
    for second in unique_seconds[:300]:
...
def generate_requests(json_file, trace_file, num_adapters, rate,
    ...
    arr_time = get_arrival_times(rate, trace_file)[:5]

    tot_req = len(arr_time)
    req_index = np.random.randint(0, 1000, size=tot_req)
    ...
    for i in range(tot_req):
        # tic += intervals[i]
        # print(ind[i])
        if req_index[i] in req_index[:i]:
            # token_counts[i][2] = 0
            requests.append(Request(
                req_id=int(i),
                model_dir=adapter_dirs[ind[i]][0],
                adapter_dir=adapter_dirs[ind[i]][1],
                prompt=dummy_prompt(int(token_counts[i][0]), 0),
                ...
                multimodal_params={
                    "images": [{'type': "base64", "data": b64} for _ in range(0)]
                }
            ))
        else:
            requests.append(Request(
...
def process_json(path="datasets/sharegpt4v_instruct_gpt4-vision_cap100k.json", is_ours=False):
    import json
...
def process_detect_json(path="datasets/airbus+cityscape1classes.json", is_ours=False):
    tokenizer = AutoTokenizer.from_pretrained(
        "../models/Qwen-VL-Chat", trust_remote_code=True
    )
    with open(path, 'r') as file:
        data = json.load(file)
    ...
    for _ in range(700):
        if is_ours or True:
            token_counts.append((0, 1, 6))
        else:
            token_counts.append((150, 7, 6))
